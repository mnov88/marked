...
def process_file(xml_file: Path, config: Dict[str, Dict[str, str]], output_root: Optional[Path], verbose: bool) -> Optional[Path]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract metadata from CELLAR case notices.")
    ...
    for xml_file in targets:
        process_file(xml_file, config, output_root, args.verbose)

    print(f"Processed {len(targets)} case notice(s).")
